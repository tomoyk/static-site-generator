      ...
      if( preg_match($ptn, $tmp) ){
        ...
        switch($foo){
          ...
          case "UPDATE_LIST":
            ...
            break;
          case "BASE_URI":
            $after = (SSL ? 'https://' : 'http://').HTTP_HOST.'/'.DOCUMENT_ROOT;
            break;
        }
        // 置換
...
function make_childList($filePath, $fileName, $mode){
  ...
  for($i=0;$i<count($pageInfo);$i++){
    ...
    if( ($pageInfo[$i]['Name']=="index.txt" && preg_match("#^".$filePath."[^\/\s]+/#", $pageInfo[$i]['Path']))
      ...
      // ソースのパスを書き込むパスに変更
      $new_fpath = (SSL ? 'https://' : 'http://').HTTP_HOST.'/'.DOCUMENT_ROOT.preg_replace("#^".DATA_PATH."/#", '', $pageInfo[$i]['Path']);
...
function make_sitemap(){
  ...
  for($i=0;$i<count($pages);$i++) {
    $new_path = (SSL ? 'https://' : 'http://').HTTP_HOST.'/'.DOCUMENT_ROOT.preg_replace("#^".DATA_PATH."/#", '', $pages[$i]['Path']);
...
function make_updateList(){
  ...
  for($i=0;$i<PRINT_UPDATE_POST;$i++){
    $new_path = (SSL ? 'https://' : 'http://').HTTP_HOST.'/'.DOCUMENT_ROOT.preg_replace("#^".DATA_PATH."/#", '', $latestPosts[$i]['Path']);
